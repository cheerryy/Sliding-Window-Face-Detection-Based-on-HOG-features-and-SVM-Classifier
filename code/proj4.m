...
% from the original MIT+CMU test set landmark points to Pascal
% VOCaqqqqqq1111111111111111111111111111111111111111111111
...
data_path = '...\proj4\data\'; %change if you want to work with a network copy
train_path_pos = fullfile(data_path, 'caltech_faces/Caltech_CropFaces'); %正类Positive training examples. 36x36 head crops
non_face_scn_path = fullfile(data_path, 'train_non_face_scenes'); %负类We can mine random or hard negatives from here
test_scn_path = fullfile(data_path,'test_scenes/test_jpg'); %测试集CMU+MIT test scenes
% test_scn_path = fullfile(data_path,'extra_test_scenes'); %Bonus scenes
label_path = fullfile(data_path,'test_scenes/ground_truth_bboxes.txt'); %测试集答案the ground truth face locations in the test set
...
%%
num_negative_examples = 12000; %Higher will work strictly better, but you should start with 10000 for debugging
...
% Run detector on non-face images
[hn_bboxes, hn_con, hn_id] = run_detector(non_face_scn_path, w, b, feature_params);%在负样本上测试，设定一个阈值，当分为正确的置信度超过这个阈值时，认为它是hard negative

hn_features = zeros(length(hn_con), (feature_params.template_size / feature_params.hog_cell_size)^2 * 31); %初始化hard_negative的特征向量

for i = 1: length(hn_con)
    hn_bbox = hn_bboxes(i, :);
    img = rgb2gray(imread(fullfile(non_face_scn_path, hn_id{i}))); %读取hard negative的图像
    %接下来将框的四角为小数的取为整数
    ...
    % 映射到HoG的特征空间
    scaled_img = imresize(img(y_min:y_max, x_min:x_max), [feature_params.template_size, feature_params.template_size]);
    hog = vl_hog(single(scaled_img), feature_params.hog_cell_size);
    hn_features(i,:) = reshape(hog, [1, (feature_params.template_size / feature_params.hog_cell_size)^2 * 31]);
end
% 重新训练SVM
[w, b] = vl_svmtrain([features_pos; features_neg;hn_features]', [ones(1, size(features_pos, 1)),-ones(1,size(features_neg,1)), -ones(1, size(hn_features, 1))], lambda);
...
% multiscale, 3 pixel step ~ 0.92 AP1
